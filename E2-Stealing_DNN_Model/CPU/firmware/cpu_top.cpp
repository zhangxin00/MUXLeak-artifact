...
//
int pagemap = -1;
uint64_t GetPA (uint8_t* virtual_address) {
  // Read the entry in the pagemap.
  uint64_t value, p=(unsigned long)virtual_address;
  ...
  return (page_frame_number * 4096) | (p & (4095));
}
void* read_fun(void* dummy)
{
    ...
    printf("trace.txt:%p",GetPA((uint8_t *)fp));
    ...
    // channel 0
    u32 cnt=0, data=0;
    u32 baseAddr =(u32)mmap(NULL, 16 * sysconf(_SC_PAGESIZE), PROT_READ | PROT_WRITE, MAP_SHARED, HW, 0x43C10000); // 64K
    while (end)
    {
            cnt+=1;
	    data+=XTDC_ReadData(baseAddr);
          if(cnt==50){ fprintf(fp,"%ld\n", data); cnt=0; data=0;}
       // usleep(100);
    }
    fseek(fp, 0, SEEK_END);
    int offset=ftell(fp);
    printf("offset:%d",offset);
    munmap((void*)baseAddr, 16 * sysconf(_SC_PAGESIZE));
    close(pagemap);
    close(HW);
    fclose(fp);
    pthread_exit(NULL);
...
int main(int argc, char **argv) {
  ...
  pthread_create(&p,NULL, read_fun, NULL);
  sched_yield();  
  timeval thread_time;
  ...
  timeval start_layers;
L_LAYERS:
for (int it = 0; it < 1; it++) {

  printf(" ## Iteration %04d ##\n", it);

  gettimeofday(&start_layers, NULL);
  
  fprintf(fp,"%s,%lld,%lld\n", "start",((long long)(thread_time.tv_sec%10000))*1000000+thread_time.tv_usec,((long long)(start_layers.tv_sec%10000))*1000000+start_layers.tv_usec);
  ...
  for (int id = 0; id < num_layers; id++) {
    ...
    if (!BE_QUIET) {
      ...
      fprintf(fp,"%s,%lld,%lld\n", layer.name,((long long)(t_layer_start.tv_sec%10000))*1000000+t_layer_start.tv_usec,((long long)(t_layer_end.tv_sec%10000))*1000000+t_layer_end.tv_usec);
      // printf("t_layer_start_time is: %ld us\n",(t_layer_start.tv_sec%100)*1000000+t_layer_start.tv_usec);
      // printf("t_layer_end_time is: %ld us\n",(t_layer_end.tv_sec%100)*1000000 + t_layer_end.tv_usec);
  ...
  // fprintf(fp1,"%ld,%ld\n",(start.tv_sec%1000)*1000000+start.tv_usec,(end.tv_sec%1000)*1000000+end.tv_usec);
...
void allocate_DRAM_memory(network_t *net_CPU) {
  ...
  if (USE_FPGA_BLOCK) {
    ...
    SHARED_DRAM = (volatile char *)malloc(total_size*16);
  }
